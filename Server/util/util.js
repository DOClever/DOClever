...
var init=async (function () {
  let setConfig=async (function () {
    if((!fs.existsSync(path.join(__dirname, "../../config.json")) || !require("../../config.json").db) && process.argv.length<=2)
    {
      ...
      let arr=["请输入mongodb数据库地址（比如：mongodb://localhost:27017/DOClever)：","请输入DOClever上传文件路径（比如：/Users/Shared/DOClever）：","请输入端口号（比如10000）：","是否启用https（比如y/n）"];
      for(let i=0;i<arr.length;i++)
      {
        ...
        {
          con.https=val.trim().toLowerCase()=="y"?true:false;
        }
      }
