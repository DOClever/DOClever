        ...
        <table width="100%" style="border-spacing: 0">
            <template v-for="(item,index) in arr">
                <tr style="text-align: center;vertical-align: middle;cursor: move;height: 50px;line-height: 50px" :draggable="item.drag?item.drag:'false'" @dragover="dragOver($event,item)" @dragleave="dragLeave($event,item)" @drop="drop($event,item,arr)" @dragstart="dragStart($event,item,index,arr)" @dragend="dragEnd($event)">
                    ...
                    <td style="width: 28%">
                        <el-input type="textarea" style="width: 90%;height: 46px;line-height: 46px" resize="none" :rows="0" placeholder="请填写备注;" v-model="item.remark" @focus="focus(item)" @blur="blur(item)" :disabled="true"></el-input>
                    </td>
                    <td style="width: 20%">
                        <el-button type="text" size="small" @click="configValue(item)"  v-if="item.type!=3 && item.type!=4" style="font-size: 15px">{{(item.value && (item.value.data.length>0 || item.value.status))?"已填值":"未填值"}}</el-button>
                    </td>
                </tr>
