<template>
    <el-dialog title="设置项目所有者"  size="small" ref="box" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false" v-model="showDialog">
            ...
            <template v-for="item in arrProject">
                <el-row class="row" style="height: 40px;line-height: 40px" :key="item._id">
                    ...
                    <el-col class="col" :span="6">
                        <el-button type="primary" v-if="item.handle==0" @click="setOwner(item)">
                            设置所有者
                        </el-button>
                        <span v-else>
                            已设置
