<template>
    <el-dialog title="指定项目所有者"  size="small" ref="box" v-model="showDialog">
                    ...
                    <el-collapse-item :title="item.name" class="hover">
                        <template v-for="(item1,index1) in item.users">
                            <el-row class="row" style="height: 40px;line-height: 40px;text-align: center">
                                ...
                                <el-col class="col" :span="3">
                                    <el-button  style="font-size: 15px" size="small" @click="transfer(item1)" type="text" v-if="item1.role==0 || item1.role==1">转让</el-button>
                                </el-col>
                            </el-row>
                        </template>
                    </el-collapse-item>
