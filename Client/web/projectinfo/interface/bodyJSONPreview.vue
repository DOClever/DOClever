<template>
    <el-row class="row" style="cursor: pointer;height: 100%">
        <table width="100%" style="border-spacing: 0;border-collapse: collapse;table-layout:fixed;word-wrap:break-word;word-break: break-all;font-size: 17px" border="1" bordercolor="#ddd">
            <tr v-if="level==0" style="text-align: center;vertical-align: middle;cursor: move;height: 40px;line-height: 40px;background-color: #20A0FF;color: white">
                <td style="width: 30%">
                    名称
                </td>
                <td style="width: 14%">
            ...
            <template v-for="(item,index) in arr">
                <tr style="text-align: center;vertical-align: middle;cursor: pointer;height: 40px;line-height: 40px">
                    ...
                    <td style="width: 28%;line-height: normal">
                        {{item.remark?item.remark:"无备注"}}
                    </td>
                    <td style="width: 20%">
                        <el-autocomplete class="inline-input" v-model="item.mock" :fetch-suggestions="querySearch" placeholder="选择参考值" icon="caret-bottom" :on-icon-click="showAutoComplete" @mouseenter.native="focusAuto(item)" style="width:100%" popper-class="my-autocomplete" custom-item="itemauto" v-if="item.value && (item.value.data.length>0 || item.value.status)"></el-autocomplete>
                    </td>
                </tr>
