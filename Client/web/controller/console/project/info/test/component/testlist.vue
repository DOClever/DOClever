<template>
    <el-tree :data="data" :props="defaultProps" show-checkbox node-key="id"  :render-content="renderContent" highlight-current ref="tree" @current-change="info" style="font-size: 14px" empty-text="暂无模块">
    </el-tree>
</template>
                ...
                return (
                        <span class="testMenu" style="display:inline-block;width:calc(100% - 45px);font-size:15px;height:26px;line-height:26px">
                            <span class="testLabel" style={{
                                ...
                                display:'inline-block',
                                fontSize: '14px',
                                color: (node.level < 3) ? '#50bfff' : (node.data.status == 0 ? 'black' : (node.data.status == 1 ? 'green' : 'red'))
                            }}>
                        ...
                        <span style="right: 5px;position:absolute;text-align: center;display:inline-block;" onclick="event.stopPropagation()">
                            {
                                node.level == 1 ?
                                        <div class="testRightMenu" style="height:26px;width;line-height: 26px;margin-right: 3px;" v-show={this.testEditRole}>
