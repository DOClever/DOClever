...
module.exports= {
    ...
    actions:{
        addBaseUrl:function (context,data) {
            if(!context.state.project.baseUrls)
            {
                context.state.project.baseUrls=[];
            }
            context.state.project.baseUrls.push($.clone(data));
            context.getters.event.$emit("addBaseUrl",data);
        },
...
if (module.hot) {
    module.hot.accept(['./global/store','./interface/store','./setting/store','./version/store'], function () {
        var global=require("./global/store");
        var inter=require("./interface/store");
        var setting=require("./setting/store");
        var version=require("./version/store");
